/**
 * Real Quality Junior School — Shared JavaScript
...
(function () {
  ...
  if (navToggle && navLinks) {
    ...
    function closeMobileNavIfWide() {
      if (window.matchMedia('(min-width: 769px)').matches && navLinks.classList.contains('open')) {
        navToggle.setAttribute('aria-expanded', 'false');
        navLinks.classList.remove('open');
        document.body.style.overflow = '';
      }
    ...
    window.addEventListener('resize', closeMobileNavIfWide);
    window.addEventListener('orientationchange', closeMobileNavIfWide);
  }
  ...
  if (bodyPage) {
    document.querySelectorAll('.nav__links a[data-nav]').forEach(function (a) {
      if (a.getAttribute('data-nav') === bodyPage) {
